...
def main(): 
    ...
    parser.add_argument('--direction_type', type=str, default='w_probe')
    ...
    for i in range(args.num_fold):
        ...
        if args.direction_type == 'wp_probe':
        ...
        elif args.direction_type == 'wp_probe_01':
            interventions = get_w_interventions_dict(top_heads, probes, head_wise_activations, num_heads, args.use_center_of_mass, args.use_random_dir, com_directions)
            def lt_modulated_vector_w_add(head_output, layer_name, start_edit_location='lt'):
                head_output = rearrange(head_output, 'b s (h d) -> b s h d', h=num_heads)
                for head, direction, proj_val_std, probe in interventions[layer_name]:
                    direction_to_add = torch.tensor(direction).to(args.device)
                    weight = 1 - probe.predict_proba(head_output[:, -1, head, :].detach().cpu().numpy())[0][1]
                    if start_edit_location == 'lt': 
                        head_output[:, -1, head, :] += args.alpha * proj_val_std * direction_to_add * weight
                    else: 
                        head_output[:, start_edit_location:, head, :] += args.alpha * proj_val_std * direction_to_add * weight
            ...
            def lt_modulated_vector_w_add(head_output, layer_name, start_edit_location='lt'):
                ...
                return head_output
            lt_modulated_fn = lt_modulated_vector_w_add
        elif args.direction_type == 'w_probe':
            interventions = get_w_interventions_dict(top_heads, probes, head_wise_activations, num_heads, args.use_center_of_mass, args.use_random_dir, com_directions)
            def lt_modulated_vector_w_add(head_output, layer_name, start_edit_location='lt'):
                head_output = rearrange(head_output, 'b s (h d) -> b s h d', h=num_heads)
                for head, direction, proj_val_std, probe in interventions[layer_name]:
                ...
                return head_output
            lt_modulated_fn = lt_modulated_vector_w_add
        elif args.direction_type == 'w_probe_v2':
            interventions = get_w_interventions_dict(top_heads, probes, head_wise_activations, num_heads, args.use_center_of_mass, args.use_random_dir, com_directions)
            def lt_modulated_vector_w_add(head_output, layer_name, start_edit_location='lt'):
                head_output = rearrange(head_output, 'b s (h d) -> b s h d', h=num_heads)
                for head, direction, proj_val_std, probe in interventions[layer_name]:
                ...
                return head_output
            lt_modulated_fn = lt_modulated_vector_w_add
        elif args.direction_type == 'w_probe_01':
            interventions = get_w_interventions_dict(top_heads, probes, head_wise_activations, num_heads, args.use_center_of_mass, args.use_random_dir, com_directions)
            def lt_modulated_vector_w_add(head_output, layer_name, start_edit_location='lt'):
                head_output = rearrange(head_output, 'b s (h d) -> b s h d', h=num_heads)
                for head, direction, proj_val_std, probe in interventions[layer_name]:
                    direction_to_add = torch.tensor(direction).to(args.device)
                    weight = 1 - probe.predict(head_output[:, -1, head, :].detach().cpu().numpy())[0]
                    if start_edit_location == 'lt': 
                        head_output[:, -1, head, :] += args.alpha * proj_val_std * direction_to_add * weight
                    else: 
                        head_output[:, start_edit_location:, head, :] += args.alpha * proj_val_std * direction_to_add * weight
        ...
        curr_fold_results = alt_tqa_evaluate(
            ...
            intervention_fn=lt_modulated_fn if not args.pure else None, 
