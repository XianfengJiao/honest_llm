...
def get_centring_interventions_dict(centring_vectors, tuning_activations, num_heads, layer_idx): 

    interventions = {}

    for head in range(num_heads):
        interventions[f"model.layers.{layer_idx}.self_attn.head_out"] = []
    for head in range(num_heads):
        direction = centring_vectors[layer_head_to_flattened_idx(layer_idx, head, num_heads)]
        direction = direction / np.linalg.norm(direction)
        activations = tuning_activations[:,layer_idx,head,:] # batch x 128
        proj_vals = activations @ direction.T
        proj_val_std = np.std(proj_vals)
        interventions[f"model.layers.{layer_idx}.self_attn.head_out"].append((head, direction.squeeze(), proj_val_std))
    for head in range(num_heads):
        interventions[f"model.layers.{layer_idx}.self_attn.head_out"] = sorted(interventions[f"model.layers.{layer_idx}.self_attn.head_out"], key = lambda x: x[0])
...
def main(): 
    ...
    parser.add_argument('--select_layer', type=int, default=25)
    ...
    print(args)
    experiment_name = f'{args.model_name}_mean_centring_layer{args.select_layer}_alpha{args.alpha}'
    ...
    for i in range(args.num_fold):
        ...
        centring_vectors = get_centring_vectors(args.seed, train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers=num_layers, num_heads=num_heads)


        interventions = get_centring_interventions_dict(centring_vectors, head_wise_activations, num_heads, layer_idx=args.select_layer)
